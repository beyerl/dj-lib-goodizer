...
QColor flagColor(djcore::FlagState s) {
  switch (s) {
    case djcore::FlagState::Ok:
      return QColor(0xE6, 0xF4, 0xEA);  // green tint
    case djcore::FlagState::Review:
      return QColor(0xFF, 0xF4, 0xCE);  // amber tint
    case djcore::FlagState::OutsideTarget:
      return QColor(0xFA, 0xE3, 0xE3);  // red tint
  }
  return {};
}
...
QVariant LibraryModel::data(const QModelIndex& index, int role) const {
  ...
  if (role == Qt::DisplayRole) {
    switch (index.column()) {
      ...
      case Loudness:
        return a ? QString::number(a->integratedLufs, 'f', 1) + " LUFS"
                 : QStringLiteral("—");
      case DynamicRange:
        return a ? QString::number(a->drValue, 'f', 1) + " dB" : QStringLiteral("—");
      case Mono:
        return a ? QString::fromLatin1(djcore::flagLabel(djcore::monoFlag(*a, profile_)))
                 : QStringLiteral("—");
      case Width:
        return a
                   ? QString::fromLatin1(djcore::flagLabel(djcore::widthFlag(*a, profile_)))
                   : QStringLiteral("—");
    }
  }
  ...
  if (role == Qt::BackgroundRole && a) {
    switch (index.column()) {
      case Loudness:
        return QBrush(flagColor(djcore::loudnessFlag(*a, profile_)));
      case DynamicRange:
        return QBrush(flagColor(djcore::dynamicRangeFlag(*a, profile_)));
      case Mono:
        return QBrush(flagColor(djcore::monoFlag(*a, profile_)));
      case Width:
        return QBrush(flagColor(djcore::widthFlag(*a, profile_)));
    }
  }
...
QVariant LibraryModel::headerData(int section, Qt::Orientation o, int role) const {
  ...
  switch (section) {
    ...
    case DynamicRange:
      return QStringLiteral("Dynamic Range");
    case Mono:
